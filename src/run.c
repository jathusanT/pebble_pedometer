...
char *item_sub[5] = {"Lets Go!", "Not Set", "Current: Light", "v1.0-RELEASE", "Jathusan T, (c) 2014"};
...
void stepGoal_load(Window *window){
	...
	if (isDark){
		...
		text_layer_set_background_color(stepGoalVisualizer, GColorWhite);
		action_bar_layer_set_background_color(stepGoalSetter, GColorWhite);
	} else {
		window_set_background_color(set_stepGoal, GColorWhite);
		text_layer_set_background_color(stepGoalVisualizer, GColorBlack);
